    ...
    serviceConfig = {
      ExecStart = ''
        ${pkgs.zapret}/bin/nfqws --pidfile=/run/nfqws.pid --qnum=200 \
          --filter-udp=443 --hostlist="${LISTS}zapret-hosts-user.txt" --hostlist-exclude="${LISTS}zapret-hosts-exclude.txt" --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fake-quic="${BIN}quic_initial_www_google_com.bin" --new \
          ...
          --filter-tcp=80,443,2053,2083,2087,2096,8443 --hostlist="${LISTS}zapret-hosts-user.txt" --hostlist-exclude="${LISTS}zapret-hosts-exclude.txt" --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fooling=ts --dpi-desync-fake-tls="${BIN}tls_clienthello_www_google_com.bin" --new \
          --filter-tcp=80,443,1024-65535 --ipset="${LISTS}ipset-game.txt" --hostlist-exclude="${LISTS}zapret-hosts-exclude.txt" --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fooling=ts --dpi-desync-fake-tls="${BIN}tls_clienthello_4pda_to.bin" --new \
          --filter-udp=1024-65535 --ipset="${LISTS}ipset-game.txt" --hostlist-exclude="${LISTS}zapret-hosts-exclude.txt" --dpi-desync=fake --dpi-desync-autottl=2 --dpi-desync-repeats=12 --dpi-desync-any-protocol=1 --dpi-desync-fake-unknown-udp="${BIN}quic_initial_www_google_com.bin" --dpi-desync-cutoff=n2
      '';
      Type = "simple";
      Restart = "always";
    };
